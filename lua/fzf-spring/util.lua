...
local H = require "fzf-spring.helper"
...
local get_grep_cmd = function(annotation)
    if annotation == E.annotation.REQUEST_MAPPING then
        return "rg '@RequestMapping(([^)]*?))' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    elseif annotation == E.annotation.GET_MAPPING then
        return "rg '@GetMapping' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    elseif annotation == E.annotation.POST_MAPPING then
        return "rg '@PostMapping' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    elseif annotation == E.annotation.PUT_MAPPING then
        return "rg '@PutMapping' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    elseif annotation == E.annotation.DELETE_MAPPING then
        return "rg '@DeleteMapping' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    elseif annotation == E.annotation.PATCH_MAPPING then
        return "rg '@PatchMapping' --multiline --type java --line-number --trim --vimgrep | grep -v RestClient"
    else
...
M.set_cursor_on_entry = function(entry, bufnr)
    vim.api.nvim_win_set_cursor(0, { tonumber(entry.line_number), tonumber(entry.column) })
